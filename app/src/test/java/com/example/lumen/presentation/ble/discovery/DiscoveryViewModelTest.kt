...
class DiscoveryViewModelTest {
    ...
    @Test
    fun `uiState correctly maps devices and ids favorites`() =
        runTest {
    ...
    @Test
    fun `uiState filters only favorites when FAVORITE_DEVICES type is selected`() =
        runTest {
    ...
    @Test
    fun `uiState provides correct empty text message for scanning favorites`() =
        runTest {
    ...
    @Test
    fun `uiState reacts to list type changes`() =
        runTest {
    ...
    @Test
    fun `init should call startScanUseCase when scan preconditions are met`() =
        runTest {
    ...
    @Test
    fun `init should call stopScanUseCase when BT is turning off and is scanning`() =
        runTest {
            scanStateFlow.emit(ScanState.SCANNING)
            viewModel.onBtPermissionResult(granted = true, showRationale = false)
    ...
    @Test
    fun `snackbar should not show when connection is established`() =
        runTest {
            viewModel.snackbarEvent.test {
                connectionResultFlow.emit(ConnectionResult.ConnectionEstablished)
                expectNoEvents()
    ...
    @Test
    fun `onBtPermissionResult updates btPermissionStatus state correctly`() =
        runTest {
            viewModel.onBtPermissionResult(granted = true, showRationale = false)
            assertEquals(
    ...
    @Test
    fun `startScan calls use case if BT is enabled and perms granted`() =
        runTest {
            viewModel.onBtPermissionResult(granted = true, showRationale = false)
            bluetoothStateFlow.emit(BluetoothState.ON)
    ...
    @Test
    fun `stopScan calls use case if BT is enabled and perms granted`() =
        runTest {
            viewModel.onBtPermissionResult(granted = true, showRationale = false)
            bluetoothStateFlow.emit(BluetoothState.ON)
    ...
    @Test
    fun `connectToDevice calls use case`() =
        runTest {
    ...
    @Test
    fun `retryConnection calls use case when device is available`() =
        runTest {
            viewModel.connectToDevice(device)
    ...
    @Test
    fun `clearInfoMessage resets state`() =
        runTest {
            connectionResultFlow.emit(ConnectionResult.Disconnected)
    ...
    @Test
    fun `clearErrorMessage resets state`() =
        runTest {
            viewModel.retryConnection()
    ...
    @Test
    fun `addFavDevice calls use case`() =
        runTest {
    ...
    @Test
    fun `removeFavDevice calls use case`() =
        runTest {
    ...
    @Test
    fun `selectDeviceListType calls save use case only when type is different`() =
        runTest {
    ...
    @Test
    fun `selectDeviceListType saves preference when switching types`() =
        runTest {
            listTypeFlow.emit(DeviceListType.ALL_DEVICES)
