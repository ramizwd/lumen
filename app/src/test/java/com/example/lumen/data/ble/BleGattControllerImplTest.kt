...
class BleGattControllerImplTest {
    ...
    @Test
    fun `full connection flow should reach STATE_LOADED_AND_CONNECTED`() =
        runTest {
    ...
    @Test
    fun `connect should set selectedDevice`() =
        runTest {
            controller.connect(device)
    ...
    fun `connect should emit error when BT is disabled`() =
        runTest {
            ...
            controller.connectionEvents.test {
                ...
                assertEquals(
                    ConnectionResult.Failure.BtDisabled,
                    awaitItem(),
                )
            }
    ...
    fun `connect should emit error when connectGatt throws an exception`() =
        runTest {
            ...
            controller.connectionEvents.test {
                assertEquals(
                    ConnectionResult.Failure.DeviceNotFound,
                    awaitItem(),
                )
            }
    ...
    @Test
    fun `onConnectionStateChange should trigger retry on failure after 500ms`() =
        runTest {
    ...
    @Test
    fun `disconnect should disconnect connected device`() =
        runTest {
            controller.connect(device)
    ...
    @Test
    fun `disconnect should cancel ongoing connection attempt`() =
        runTest {
            controller.connect(device)
    ...
    @Test
    fun `disconnect should cancel ongoing retry connection attempt`() =
        runTest {
            val controllerWithScope =
                BleGattControllerImpl(
                    context,
    ...
    @Test
    fun `disconnect should cancel ongoing loading state attempt and disconnect`() =
        runTest {
            val controllerWithScope =
                BleGattControllerImpl(
                    context,
    ...
    @Test
    fun `writeCharacteristic should call GATT writeCharacteristic`() =
        runTest {
            controller.connect(device)
    ...
    @Test
    fun `handle invalid device flow and ensure state is reset for subsequent connections`() =
        runTest {
    ...
    @Test
    fun `onServicesDiscovered should close connection on GATT failure`() =
        runTest {
            controller.connect(device)
    ...
    @Test
    fun `onCharacteristicChanged with insufficient bytes should not update state`() =
        runTest {
    ...
    @Test
    fun `onDescriptorWrite should request controller state when CCCD is written successfully`() =
        runTest {
    ...
    @Test
    fun `onDescriptorWrite should not request state if status is failure`() =
        runTest {
            controller.connect(device)
